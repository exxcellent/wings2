...
public class SScrollPane
    ...
    private boolean isAdjustableVisible(int policy, int maxRecords, int maxDisplayed) {
        return isPolicyAlways(policy)
                || (isPolicyAsNeeded(policy) && maxRecords > maxDisplayed);
    }

    private boolean isPolicyAlways(int policy) {
        return policy == HORIZONTAL_SCROLLBAR_ALWAYS
                || policy == VERTICAL_SCROLLBAR_ALWAYS;
    }

    private boolean isPolicyAsNeeded(int policy) {
        return policy == HORIZONTAL_SCROLLBAR_AS_NEEDED
                || policy == VERTICAL_SCROLLBAR_AS_NEEDED;
    }
