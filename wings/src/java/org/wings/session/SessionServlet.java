...
final class SessionServlet
    ...
    private final static Log log = LogFactory.getLog(SessionServlet.class);
    private final static Log apperrorlog = LogFactory.getLog(SessionServlet.class.getPackage().getName()+".AppErrors");
    ...
    public final synchronized void doGet(HttpServletRequest req,
        ...
        try {
            ...
            if (defaultFrame != null && defaultFrame.handleError(e)) {
                apperrorlog.info("exception, trying to handle it via SFrame.handleError(): ", e);
                // maybe just call defaultFrame.write(new ServletDevice(out)); instead of doGet()
                doGet(req, response);
                isErrorHandling = true;
            } else {
                apperrorlog.error("exception handling failed: ", e);
