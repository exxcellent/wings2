...
public class XTreeTableModel extends DefaultTableModel {
    ...
    private final static transient Log logger = LogFactory.getLog(XTreeTableModel.class);

    private final TreeModel treeModel;

    private final List<XTreeTableNode> nodeModel = new ArrayList<XTreeTableNode>();
    ...
    public void toggleExpanded(int row) {
        ...
        XTreeTableNode node = getNodeModel().get(row);
    ...
    private int collapseNode(XTreeTableNode node, int row) {
        ...
        while ( row + 1 < getNodeModel().size() ) {
            XTreeTableNode candidate = getNodeModel().get(row + 1);
            /*
    ...
    public final Object getValueAt(int row, int column) {
        if (row >= 0 && row < getNodeModel().size()) {
            ...
            return getNodeModel().get(row).getValueAt(column);
        }
        return null;
    }
...
//            if (column > 0) {
                ...
                getNodeModel().get(row).setValueAt(aValue, column);
//            }
        }
     ...
     */
    public List<XTreeTableNode> getNodeModel() {
